...
示例 15.1 [server.go](examples/chapter\_15/server.go)
...
示例 15.2 [client.go](examples/chapter\_15/client.go)
...
示例 15.3 [dial.go](examples/chapter\_15/dial.go)
...
示例 15.4 [socket.go](examples/chapter\_15/socket.go)
...
编写新版本的客户端和服务器 ([client1.go](exercises/chapter\_15/client1.go) / [server1.go](exercises/chapter\_15/server1.go))：

* 增加一个检查错误的函数 `checkError(error)`；讨论如下方案的利弊：为什么这个重构可能并没有那么理想？看看在 [示例 15.14](examples/chapter\_15/template\_validation.go) 中它是如何被解决的
...
下边这个版本的 simple\_tcp\_server.go 从很多方面优化了第一个 tcp 服务器的示例 server.go 并且拥有更好的结构，它只用了 80 行代码！

示例 15.5 [simple\_tcp\_server.go](examples/chapter\_15/simple\_tcp\_server.go)：
...
（**译者注：应该是由于 Go 版本的更新，会提示 os.EAGAIN undefined，修改后的代码：**[**simple\_tcp\_server\_v1.go**](examples/chapter\_15/simple\_tcp\_server\_v1.go)）
