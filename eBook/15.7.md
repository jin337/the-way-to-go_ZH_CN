...
示例 15.13 [template\_field.go](examples/chapter\_15/template\_field.go)
...
示例 15.14 [template\_validation.go](examples/chapter\_15/template\_validation.go)
...
练习 15.7 [template\_validation\_recover.go](exercises/chapter\_15/template\_validation\_recover.go)

在上述示例代码上实现 defer/recover 机制。

## 15.7.3 `If-else`

运行 `Execute()` 产生的结果来自模板的输出，它包含静态文本，以及被 `{{}}` 包裹的称之为_管道_的文本。例如，运行这段代码（示例 15.15 [pipline1.go](examples/chapter\_15/pipeline1.go)）：
...
示例 15.16 [template\_ifelse.go](examples/chapter\_15/template\_ifelse.go)
...
示例 15.17 [template\_with\_end.go](examples/chapter\_15/template\_with\_end.go)
...
示例 15.18 [template\_variables.go](examples/chapter\_15/template\_variables.go)
...
示例 15.19 [predefined\_functions.go](examples/chapter\_15/predefined\_functions.go)
